...
const User = () => {
  ...
  useEffect(() => {
    ...
    if (Cookies.get("user") === undefined) {
      ...
      Cookies.set("website", "khaled-adel");
    }
  ...
  return (
    <div className="home-screen ">
      ...
      {/* top */}
      <div className="relative flex flex-col items-center justify-center gap-3 my-5 h-1/3 ">
        <div
          onClick={() => setNotiModel(true)}
          className="absolute z-10 flex items-center justify-center h-8 gap-3 transition-all rounded-full cursor-pointer top-3 left-6 hover:scale-125"
        >
        ...
        <div className="relative">
          <div className="absolute flex flex-col justify-around w-20 -translate-y-1/2 top-1/2 -right-8 h-36">
            <div
              onClick={() => {
                setClose(true);
              }}
              className="flex items-center justify-center w-8 h-8 transition-all bg-red-400 rounded-full cursor-pointer hover:scale-125"
            >
              <img className="w-5 " src={userRemove} alt="" />
            </div>
            <div
              onClick={() => setOpenModel(true)}
              className="flex items-center justify-center w-8 h-8 transition-all translate-x-3 bg-red-400 rounded-full cursor-pointer hover:scale-125"
            >
          ...
          <div className="z-10 overflow-hidden rounded-full w-28 h-28 ">
            <img
        ...
        <h1 className="z-10 text-2xl text-white">{state && state.name}</h1>
        <p className="z-10 text-xs text-white opacity-70 ">
          {state && state.email}
        </p>
      </div>
      {/* main */}

      <div className="flex flex-col items-center justify-center gap-5 mx-5 mt-9 ">
        {/* toggel */}
        ...
        {/* content */}
        {choosed === 1 ? (
          <div className="flex flex-col gap-4 p-5 border rounded-md bg-lightGray border-gray ">
            ...
            <div className="flex items-center justify-between gap-7 ">
              ...
              <a
                href={`khaled-adel.e3lanotopia.software/api/th/relogin/${
                  state && state.email
                }`}
              >
                تسجيل الخروج من الاجهزة
              </a>
            </div>
            <div className="flex items-center justify-between gap-7 ">
              <h1>رقم التلفون</h1>
              ...
              >
                ...
                <div className="w-full md:w-1/3">
                  <input
                    ...
                    value={searchText}
                    placeholder="بحث ....."
                    className="w-full px-5 py-1 text-base border outline-none back-icon focus:border-gray border-lightGray rounded-2xl "
                    type="text"
                  />
                </div>
                <div className="flex items-center justify-end flex-grow">
                  <button
                    type="submit"
                    className="gap-3 px-3 py-2 mt-3 text-base font-medium text-left text-white rounded-full cursor-pointer bg-buttonBlue"
                  >
