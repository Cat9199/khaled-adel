...
const Navbar = () => {
  ...
  return (
    <div className="z-20 flex items-center justify-between w-full h-16 px-8 bg-white nav-bar">
      {/* منصتي */}
      ...
      <div className="flex gap-2 lg:flex-grow">
        ...
        <select
          className="pr-3 transition-all rounded-md cursor-pointer bg-lightGray hover:bg-gray"
          defaultValue={Cookies.get("website")}
          onChange={websiteHandler}
          // disabled={typeof param.email !== "string"}
        >
          <option className="cursor-pointer hover:bg-gray" value="khaled-adel">
            khaled-adel
          </option>
        </select>
      </div>
      ...
      <div
        ...
        className="relative flex items-center justify-center w-8 h-8 rounded-full cursor-pointer notify bg-gray"
      >
        <div className="absolute flex items-center justify-center w-5 h-5 text-xs text-white bg-red-500 rounded-full -top-3 -right-3">
          {count}
        </div>
        <svg
